...
app = Flask(__name__)
app.secret_key = os.urandom(24)  # Flask에서 세션 사용하려면 비밀키 지정
...
def chatbot():
    ...
    if request.method == "POST":
        ...
        cust_id = request.json.get("custId")  # POST로 고객 ID도 받음

        # cust_id가 바뀌면 대화 기록 초기화
        if cust_id_tmp != cust_id:
            cust_id_tmp = cust_id  # 새로운 cust_id로 업데이트
            conversation_chain = ConversationChain(llm=llm, memory=ConversationBufferMemory())  # 대화 메모리 초기화
...
def few_shot():
    content = (
        ...
        "사이트 이용과 관련 없는 질문은 max_token = 50이야!"
    ).format(
